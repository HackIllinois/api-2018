...
function AuthService(ctx) {
   ...
   */
  this.verify = (token) => _Promise.try(() =>
  ...
  this.getGitHubSessionCodeURL = () => GITHUB_AUTH_REDIRECT + config.auth.github.id;

  this.requestGitHubAccessToken = (code) => {
    let token;
    let githubHandle;
    ...
    return request({
      ...
      .then((body) => {
        ...
        return this.getGitHubAccountDetails(token);
      })
      .then((handle) => {
        githubHandle = handle;
      ...
      .then((user) => {
        if (_.isNull(user)) {
          return this.getGitHubAccountEmail(token)
  ...
  this.getGitHubAccountDetails = (authToken) => request({
      uri: GITHUB_USER_URL,
      qs: {
  ...
  this.getGitHubAccountEmail = (authToken) => request({
