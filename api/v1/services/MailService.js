...
function MailService(ctx) {

  let config = ctx.config();
  let logger = ctx.logger();

  let client = new SparkPost(config.mail.key || 'NONE');
  let transmissionsAsync = _Promise.promisifyAll(client.transmissions);
  let recipientsAsync = _Promise.promisifyAll(client.recipientLists);
  ...
  this.checkIfSent = checkIfSent;
  this.markAsSent = markAsSent;
  if (client.isEnabled) {
    this.send = send;
    this.sendToList = sendToList;
  } else {
    this.send = sendDisabled;
    this.sendToList = sendDisabled;
  }
}
